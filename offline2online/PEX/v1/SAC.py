...
class agent(object):
    ...
    def Offlinetrain(self,sample):
        ...
        #updata  actor
        ####################
        target_q1,target_q2=self.target_Q1(state,action),self.target_Q2(state,action)
        
        expected_q_value = torch.min(target_q1,target_q2)
        ...
        next_q_value = self.reward_scale*reward + mask * self.dicount * next_value
        ...
        ####################
        for (target_param1, param1), (target_param2, param2) in zip(zip(self.target_Q1.parameters(), self.Q1.parameters()), 
                                                                    zip(self.target_Q2.parameters(), self.Q2.parameters())):
            target_param1.data.copy_(target_param1.data * (1.0 - self.tau) + param1.data * self.tau)
            target_param2.data.copy_(target_param2.data * (1.0 - self.tau) + param2.data * self.tau)
    ...
    def train(self,sample):
        ...
        target_q1,target_q2=self.target_Q1(state,actions),self.target_Q2(state,actions)
