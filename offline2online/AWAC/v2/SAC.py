...
class Hyperparameters:
    ...
    #AWAC
    awr_weight: float = 1.0   
    beta: float = 1.0    
    weight_loss: bool = True
    expectile: float = 0.7
    
    # Q Model
    Q_hidden_sizes: List[int] = field(default_factory=lambda: [256] * 4)
    Q_activ: Callable = F.relu
    Q_lr: float = 3e-4
    Q_out: int = 1

    # Actor Model
    actor_hidden_sizes: List[int] = field(default_factory=lambda: [256] * 4)
    actor_activ: Callable = F.relu
    actor_lr: float = 3e-4
    actor_log_std_min: int = -6
    actor_log_std_max: int = 0
    
    # Value model
    value_hidden_sizes: List[int] = field(default_factory=lambda: [256] * 4)
...
class agent(object):
    ...
    def Offlinetrain(self,sample):
        ...
        if self.weight_loss:
            weights = F.softmax(adv / self.beta, dim=0)
            weights = weights[:, 0]
            actor_loss = self.awr_weight*(-old_logprob*len(weights)*weights.detach()).mean()
        else:
            actor_loss = self.awr_weight*(-old_logprob).mean()
