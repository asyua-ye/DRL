...
def train_offline(RL_agent, replaybuffer, eval_env,start_time,writer, args):
    ...
    for t in range(int(args.OFFline)):
        ...
        actor_loss,q_loss = RL_agent.Offlinetrain(replaybuffer.sample())
        writer.add_scalar('offline_action_loss', actor_loss, global_step=t)
        writer.add_scalar('offline_q_loss', q_loss, global_step=t)
        
        
        
def train_online_to_offline(RL_agent,offlinereplaybuffer,onlinereplaybuffer,priorreplaybuffer,env, eval_env, offlineEnv, args):
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if t%allowTrain_time!=0 or t==0:
            ...
            priorreplaybuffer.add(state, action, next_state, reward, done)
            state = next_state
            ...
            for i in range(int(allowTrain_time)):
                actor_loss,q_loss,weight_loss,weights = RL_agent.train(onlinereplaybuffer.sample(),offlinereplaybuffer.sample(),
                                                        priorreplaybuffer.sample())
                priorreplaybuffer.update(weights)
                ti = t%allowTrain_time + i
                writer.add_scalar('action_loss', actor_loss, global_step=ti)
                writer.add_scalar('q_loss', q_loss, global_step=ti)
                writer.add_scalar('weight_loss', weight_loss, global_step=ti)
                maybe_evaluate_and_print(RL_agent, eval_env, evals, ti, start_time, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--OFFline", default=1000e3, type=int)
    ...
    parser.add_argument("--max_timesteps", default=250e3, type=int)
    ...
    priorreplaybuffer = buffer.priorReplayBuffer(state_dim,action_dim)
    offlineEnv = None
    ...
    if not args.NoToffline:
        ...
        data = d4rl.qlearning_dataset(offlineEnv)
        offlinereplaybuffer.load_D4RL(data)
        priorreplaybuffer.load_D4RL(data)
        p = 0.5
        M = offlinereplaybuffer.size
        N = 1000
        priorreplaybuffer.max = M * p / ((1-p) * N)
    
    
    
    
    train_online_to_offline(RL_agent,offlinereplaybuffer,onlinereplaybuffer,priorreplaybuffer,env, eval_env, offlineEnv,args)
    RL_agent.save(f"./{args.file_name}/models/online/")