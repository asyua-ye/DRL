...
class agent(object):
    ...
    def Offlinetrain(self,sample):
        ...
        with torch.no_grad():
            ...
            target_values = next_target_minq - self.alpha * next_logprob
            target_values = next_target_minq
            y = reward + mask * self.dicount * target_values
        ...
        if self.eta>0:
            ...
            grad_loss = torch.var(action_gradient,dim=0).sum(-1)
            grad_loss = self.eta * grad_loss
            
            q_loss -=  grad_loss.sum()
